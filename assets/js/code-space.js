...
const CodeSpaces = () => {
  ...
  const fetchAllProblems = async () => {
    const reqURL = "http://16.171.55.5/polls/problem_list";
    try {
  ...
  const handleFetchProblemDetails = async (id) => {
    const reqURL = `http://16.171.55.5/polls/problem/${id}`;
    try {
      const res = await axios.get(reqURL);
  ...
  const handleFetchAllUserAttempts = async () => {
    const reqURL = `http://16.171.55.5/polls/user_attempts/${userD.id}`;
    try {
      const res = await axios.get(reqURL);
  ...
  const handleCreateProblem = async (values) => {
    const reqURL = `http://16.171.55.5/polls/create_problem`;
    try {
