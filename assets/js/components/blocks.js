...
export const RenderIde = (props) => {
  ...
  const handleSubmitCode = async () => {
    const reqURL = `http://0.0.0.0:8000/polls/compile_code_by_pid`;
    try {
  ...
  const handleFetchUserHistoryForProblem = async () => {
    const reqURL = `http://0.0.0.0:8000/polls/user_sol`;
    const form = new FormData();
    const data = {
      pid: currentSelectedProblemInfo?.id,
      uid: userD.id,
  ...
  const handleSubmitRating = async () => {
    try {
      const reqURL = `http://0.0.0.0:8000/polls/rate_problem`;
      const form = new FormData();
      const data = {
        pid: currentSelectedProblemInfo?.id,
        uid: userD.id,
